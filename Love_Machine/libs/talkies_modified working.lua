...
function Talkies.say(title, messages, config)
  ...
  local newDialog = {
	...
	hcenter 		   = config.hcenter or false,
...
function Talkies.draw()
  ...
  -- Message title
  if currentDialog.title == "" then love.graphics.setColor(1,1,1,0) else
  love.graphics.setColor(currentDialog.backgroundColor) end
  ...
  if currentDialog.hcenter == true then 
	local width, wrappedtext = currentDialog.font:getWrap( currentMessage.visible, boxW - imgW - (4 * currentDialog.padding))
	local height = #wrappedtext * currentDialog.font:getHeight( )
	textY = textY + boxH/2 - height/2
  love.graphics.printf(currentMessage.visible, textX, textY, boxW - imgW - (4 * currentDialog.padding), currentDialog.align)

  elseif currentDialog.hcenter == false then
  love.graphics.printf(currentMessage.visible, textX, textY, boxW - imgW - (4 * currentDialog.padding), currentDialog.align)
  end
  ...
  if currentDialog:showOptions() and currentMessage.complete then
    ...
    for k, option in pairs(currentDialog.options) do
      love.graphics.print(option[1], optionLeftPad+textX+currentDialog.padding, optionsY+((k-1)*currentDialog.fontHeight +5 )) --TODO CHANGE last NUMBER
    end
    love.graphics.print(
