...
function minigame_4:init()
	...
	self.scene_name = "DO NOT grab the last bite"

	self.phase = "intro"

	self.bg_noise = love.audio.newSource( "scenes/minigame_4/sound/cafe_ambiance.ogg", "stream")
	self.noise_volume = 0.25
	self.bg_noise:setVolume(self.noise_volume)

	self.bg_music = love.audio.newSource( "scenes/minigame_4/sound/dinner_bg.ogg", "stream")
	self.music_volume = 0.35
	self.bg_music:setVolume(self.music_volume)

	self.sound_bites = { love.audio.newSource( "scenes/minigame_4/sound/bite_1.ogg", "static"),
						 love.audio.newSource( "scenes/minigame_4/sound/bite_2.ogg", "static"),
					  	 love.audio.newSource( "scenes/minigame_4/sound/bite_3.ogg", "static"),
				   		 love.audio.newSource( "scenes/minigame_4/sound/bite_4.ogg", "static")}
	self.bite_volume = 0.85
	for _, v in pairs(self.sound_bites) do
		v:setVolume(self.bite_volume)
	end
	...
	-- Taking turns
	local choices = {"player_1", "player_2"}
	self.turn = choices[math.random(2)]
...
love.graphics.setCanvas(self.canvas)
		...
		if self.phase == "intro" then
			...
			love.graphics.printf("DO NOT grab the last bite", 0,  global_height/2 - self.font:getHeight()/2 + 3, global_width + 3, "center")
			love.graphics.setColor(1,0.4,0.2)
			love.graphics.setFont(self.font)
			love.graphics.printf("DO NOT grab the last bite",0,  global_height/2 - self.font:getHeight()/2, global_width, "center")
		end
		...
		--outro and fade to black
				...
				if self.outro_timer_max - self.outro_timer <=1 then
					...
					self.bg_noise:setVolume(self.noise_volume * (self.outro_timer_max - self.outro_timer)) --fade music away
				end
			end
		 end
