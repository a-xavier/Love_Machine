...
function Runner:init(x, y, w, h, number)
	...
	self.player.original_y = self.player.y
	self.player.mass = 750
	self.player.vely = 0
	self.player.max_vely = -5000
	self.gravity = 9.81
	...
	self.jump.timer = 0
	self.jump.timer_max = 0.20
	self.jump.force = -16750
	self.jump.max_height = self.area.h - self.player.h
	...
	if self.player.stopped == false then
		...
		if love.keyboard.isDown(self.player.key) and (self.jump.has_jump == true) and (self.player.stopped == false) then
			...
			if (self.jump.timer <= self.jump.timer_max ) and (self.jump.has_jump == true) then
				...
				self.player.vely = 0
				self.player.y = self.player.original_y - self.jump.max_height * (math.pow(self.jump.timer,0.4)/math.pow(self.jump.timer_max,0.4))
			end
		else
			self.jump.jump_signal = false
		end
...
function Runner:draw()
	...
	--DEBUG LINE HEIGHTS
	love.graphics.setColor(0.5,0.5,0.5)
	for i = 0, self.area.h, self.area.h/10 do
		--love.graphics.line(self.area.x, self.area.y + i, self.area.x + self.area.w, self.area.y + i)
	end
