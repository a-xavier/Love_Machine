...
function Player:init(x, y, player)
	...
	self.img.quads = {love.graphics.newQuad(0, 0, self.area.w, self.area.h, self.img.spritesheet:getWidth(), self.img.spritesheet:getHeight() ),
				 love.graphics.newQuad(self.area.w, 0, self.area.w, self.area.h, self.img.spritesheet:getWidth(), self.img.spritesheet:getHeight() ),
				 love.graphics.newQuad(2*self.area.w, 0, self.area.w, self.area.h, self.img.spritesheet:getWidth(), self.img.spritesheet:getHeight() )}
...
function Player:hide_buttons(dt)
	if self.choice_animations_done == false then
		if self.choice_animations_tweens_generated == false then 
			table.insert(self.choice_animations_tweens, tween.new(self.choice_animations_timer_max, self.buttons.green, {x = self.buttons.red.x, alpha = 0}, "inBack"))
			table.insert(self.choice_animations_tweens, tween.new(self.choice_animations_timer_max, self.buttons.blue, {x = self.buttons.red.x, alpha = 0}, "inBack"))
			table.insert(self.choice_animations_tweens, tween.new(self.choice_animations_timer_max+0.5, self.buttons.red, {alpha = 0}))
